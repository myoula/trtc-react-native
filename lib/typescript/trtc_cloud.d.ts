...
import { TRTCParams, TRTCVideoEncParam, TRTCSwitchRoomConfig, AudioMusicParam, TRTCAudioRecordingParams, TRTCPublishCDNParam, TRTCCloudDef, TXVoiceChangerType, TXVoiceReverbType, TXSystemVolumeType, TRTCNetworkQosParam, TRTCTranscodingConfig, TRTCRenderParams } from './trtc_cloud_def';
...
export default class TRTCCloud {
    ...
    创建 TRTCCloud 单例。
    */
    static sharedInstance(): TRTCCloud;
    /**
    销毁 TRTCCloud 单例。
    */
    static destroySharedInstance(): void;
    /**
     * 添加事件
     * @param event
     * @param listener
    ...
    */
    ...
    @param param	画面渲染参数。
    */
    setRemoteRenderParams(userId: string, streamType: number, param: TRTCRenderParams): Promise<void>;
    /**
    - 设置视频编码器相关参数
    ...
    */
    updateSelf3DSpatialPosition(position: number[], axisForward: number[], axisRight: number[], axisUp: number[]): Promise<void>;
    /**
    - 设置 3D 音效中远端用户坐标信息。
    ...
    */
    updateRemote3DSpatialPosition(userId: string, position: number[]): Promise<void>;
    /**
    - 设置指定用户所发出声音的可被接收范围。
...
export { TXVideoView, TRTCParams, TRTCCloudDef, TXVoiceChangerType, TXVoiceReverbType, TRTCCloudListener, TRTCSwitchRoomConfig, TRTCPublishCDNParam, TRTCAudioRecordingParams, AudioMusicParam, TXSystemVolumeType, TRTCVideoEncParam, TRTCNetworkQosParam, TRTCTranscodingConfig, };
